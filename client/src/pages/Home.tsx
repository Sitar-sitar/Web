...
type PartyRecommendation = { id: string; rank: 1 | 2 | 3; title: LocalizedText; members: Array<{ name: LocalizedText; role: LocalizedText }>; synergy: LocalizedText[]; targetChanges: PartyTargetChange[]; targetSummary: LocalizedText; gameVersion: string; dataAsOf: string; updatedAt: string; sourceLabel: LocalizedText; sourceUrl: string };
...
function partyAdjustedComparisons(base: Comparison[], stats: Character["allStats"], changes: PartyTargetChange[], language: "ja" | "en" | "zh-CN"): Comparison[] {
  ...
  const adjusted = base.map((comparison) => {
    ...
    return { ...comparison, label: localText(change.label, language), unit: change.unit, targets: change.targets, achieved: {
      "厳選": comparison.current === null ? null : comparison.current >= change.targets["厳選"],
      "目標": comparison.current === null ? null : comparison.current >= change.targets["目標"],
      "妥協": comparison.current === null ? null : comparison.current >= change.targets["妥協"],
    } };
  });
  ...
  changes.filter((change) => !existing.has(change.key)).forEach((change) => {
    ...
    adjusted.push({ key: change.key, label: localText(change.label, language), unit: change.unit, current, currentDisplay: stat?.display ?? "未取得", targets: change.targets, achieved: {
      "厳選": current === null ? null : current >= change.targets["厳選"],
      "目標": current === null ? null : current >= change.targets["目標"],
      "妥協": current === null ? null : current >= change.targets["妥協"],
    } });
  });
  return adjusted;
}
...
function recommendationsFromComparisons(comparisons: Comparison[], language: "ja" | "en" | "zh-CN"): PriorityRecommendation[] {
  return comparisons
    .filter((comparison): comparison is Comparison & { current: number } => comparison.current !== null && comparison.current < comparison.targets["目標"])
    .map((comparison) => {
      ...
      return {
        ...
        priority,
        rationale: language === "ja" ? `目標 ${target}${comparison.unit} まであと ${deficit.toFixed(comparison.unit === "%" ? 1 : 0)}${comparison.unit}` : `${target}${comparison.unit} / -${deficit.toFixed(comparison.unit === "%" ? 1 : 0)}${comparison.unit}`,
      };
    })
...
export default function Home() {
  ...
  const activeComparisons = useMemo(() => partyAdjustedComparisons(selected?.comparisons ?? [], selected?.allStats ?? [], selectedParty?.targetChanges ?? [], language), [language, selected?.allStats, selected?.comparisons, selectedParty?.targetChanges]);
  ...
  const partyUi = language === "ja"
    ? { eyebrow: "PARTY SYNERGY / 最大3案", title: "おすすめPT編成", select: "編成を選択", members: "編成メンバー", synergy: "シナジー", targets: "この編成での目標変更", version: "対応バージョン", dataAsOf: "編成基準日", updated: "最終更新日", source: "編成の参照", noData: "このキャラクターの編成データは現在準備中です。目標ステータスは個別ガイドの基本値で表示します。", battleNote: "戦闘中・条件付きのバフは公開プロフィールの現在値へ加算せず、目標変更と説明で分けて表示します。" }
    : language === "zh-CN"
      ? { eyebrow: "队伍协同 / 最多3套", title: "推荐队伍", select: "选择队伍", members: "队伍成员", synergy: "协同效果", targets: "此队的目标变更", version: "适用版本", dataAsOf: "队伍基准日", updated: "最后更新日", source: "队伍参考", noData: "该角色的队伍数据正在准备中。目标属性将使用角色指南的基础值显示。", battleNote: "战斗内和条件性增益不会计入公开面板当前值，而会通过目标变更和说明单独显示。" }
      : { eyebrow: "PARTY SYNERGY / UP TO 3", title: "Recommended Teams", select: "Select a team", members: "Team members", synergy: "Synergy", targets: "Targets for this team", version: "Game version", dataAsOf: "Team data as of", updated: "Last updated", source: "Team reference", noData: "Team recommendations for this character are being prepared. Target stats use the character guide baseline.", battleNote: "In-combat and conditional buffs are not added to public-profile stats; they are shown separately through target changes and notes." };
