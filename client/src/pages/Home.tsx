...
export default function Home() {
  ...
  return <div className="min-h-screen overflow-hidden">
    ...
    <main className="container pb-20 pt-10 sm:pt-16">
      ...
      {characters.length > 0 && <><section className="mt-12 border-y border-stone-400 py-3"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><p className="detail-mono text-[9px] text-stone-500">ARCHIVE / {query.data?.player.name}</p><span className="h-1 w-1 rounded-full bg-stone-400" /><p className="detail-mono text-[9px] text-stone-500">UID {query.data?.player.uid}</p></div><div className="flex items-center gap-2 text-[10px] text-stone-500"><Clock3 className="h-3.5 w-3.5" />{query.data?.cached ? "キャッシュ" : "更新"} {formatTime(query.data?.fetchedAt)}</div></div></section>
