...
type Character = {
  ...
  statsNote?: string;
  guide: { headline: string; relicSet: string; planarSet: string; mainStats: Array<{ slot: string; value: string }>; targetContext?: string };
...
export default function Home() {
  ...
  return <div className="min-h-screen overflow-hidden">
    ...
    <main className="container pb-20 pt-10 sm:pt-16">
      ...
      {!lookupUid && !query.isFetching && <section className="mt-16 grid gap-8 border-t border-stone-400 pt-6 lg:grid-cols-[.7fr_1.3fr]"><p className="detail-mono text-[10px] text-stone-500">HOW IT WORKS / 01—03</p><div className="grid gap-6 sm:grid-cols-3">{[["01", "公開設定", "ゲーム内プロフィールに確認したいキャラクターを登録します。"], ["02", "UIDを入力", `${activeGame.uidHint}を入力して、公開データを照会します。`], ["03", "次の一手", `${activeGame.setPrimary}と各目標水準を比較し、未達ステータスを見つけます。`]].map(([number, title, body]) => <article key={number} className="border-t border-stone-300 pt-3"><p className="detail-mono text-[9px] text-amber-800">{number}</p><h3 className="mt-3 font-serif text-lg font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-stone-600">{body}</p></article>)}</div></section>}
      {characters.length > 0 && <><section className="mt-12 border-y border-stone-400 py-3"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><p className="detail-mono text-[9px] text-stone-500">ARCHIVE / {query.data?.player.name}</p><span className="h-1 w-1 rounded-full bg-stone-400" /><p className="detail-mono text-[9px] text-stone-500">UID {query.data?.player.uid}</p>{game === "hsr" && query.data?.dataSource === "Enka" && <><span className="h-1 w-1 rounded-full bg-amber-700" /><p className="detail-mono text-[9px] text-amber-800">ENKA FALLBACK</p></>}</div><div className="flex items-center gap-2 text-[10px] text-stone-500"><Clock3 className="h-3.5 w-3.5" />{query.data?.cached ? "キャッシュ" : "更新"} {formatTime(query.data?.fetchedAt)}</div></div></section>
          ...
          <div className="grid gap-7 lg:grid-cols-[.78fr_1.22fr]">
            <aside className="relative min-h-[390px] overflow-hidden bg-stone-900 p-6 text-stone-50 sm:p-8">
              ...
              <div className="relative flex h-full min-h-[340px] flex-col justify-between">
                <div className="flex justify-between"><p className="detail-mono text-[9px] text-stone-300">SELECTED RECORD</p><p className="detail-mono text-[9px] text-stone-300">LV.{selected.level ?? "—"} / {activeGame.characterRank}{selected.rank ?? 0}</p></div>
